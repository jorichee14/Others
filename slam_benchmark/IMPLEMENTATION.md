...
| I2 | **The three board dwell windows** — candidates MEASURED by phase0 (2026-09-19), in `configs/coop2.yaml` beside each anchor, awaiting eye confirmation. Coverage: `mobile_1` gets `anchor` (start) + `rs_anchor` (end) but **never visits `anchor_b`** — its tier-2 check is two points, not three; `mobile_2` gets all three. The agents swap corners over the run. — when each platform stood at `anchor`, `anchor_b`, `rs_anchor`, and at what offset. | ⬜ open | Without them tier 2 is silent and no independent evidence enters the benchmark. The single highest-value open item. |
...
| I13 | **The ZED camera↔IMU extrinsic.** | ⬜ open — and **NOT in the bag**: phase0 dumped all 8 static transforms; the ZED chain stops at the optical frames and the only IMU transform is `os_sensor→os_imu` (the Ouster's, excluded). SDK / `SN*.conf` route only. | Three routes, cheapest first: the SDK's `getSensorsConfiguration().camera_imu_transform`; the factory `SN<serial>.conf`; or `/tf_static` — whose 3 messages are the *expected* shape, one array per static publisher, not a shortfall. The trap is the body↔optical convention change between `zed_camera_center` and `zed_left_camera_optical_frame`. |
