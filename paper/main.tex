...
\title{CoPSC: A Multi-Agent Dataset for Cooperative Perception, Sensing, and Communication\\
...
once. We present CoPSC, a multi-agent, multi-view, multi-sensor dataset that
...
% layers jointly, no dataset provides that, and CoPSC does.
...
We close this gap with \textbf{CoPSC}, a multi-agent, multi-view,
multi-sensor dataset that jointly captures all three layers plus dense
reconstruction supervision. The main contributions of this paper are:
\begin{itemize}
...
\subsection{Positioning of CoPSC}
Table~\ref{tab:comparison} summarizes the landscape. Each prior family
covers at most two of the three layers; CoPSC is, to our knowledge, the
...
\begin{table}[htbp]
\caption{Comparison of CoPSC With Existing Datasets}
\begin{center}
\begin{tabular}{lcccc}
\toprule
...
\textbf{CoPSC (ours)}              & \cmark & \cmark & \cmark & \cmark \\
\bottomrule
\end{tabular}
...
%=======================================================================
\section{The CoPSC Dataset}
\label{sec:dataset}
The CoPSC system focuses on bridging the gap between sensing and
...
We presented CoPSC, a multi-agent dataset that jointly captures
