...
% Pattern (M2DGR \cite{m2dgr}): keep sensing/calibration sections unified;
% express the indoor/outdoor split via (a) a sequence-inventory table with a
% per-sequence ground-truth-source column and (b) per-environment GT
% paragraphs in Sec.~\ref{sec:groundtruth}. Indoor-outdoor *transition*
% sequences are worth capturing as first-class scenarios: for SCOOPS the
% radio channel also changes character at the boundary (LoS to NLoS).
...
(LoS/NLoS, multipath). Add a site-overview figure and a sequence-inventory
table (scenario, condition, duration, GT source), following M2DGR
\cite{m2dgr}.}
...
% Structure as per-environment paragraphs with one accuracy number each,
% as in M2DGR \cite{m2dgr} (mocap indoors, laser tracker in halls, RTK/INS
% outdoors). If indoor GT covers only parts of a trajectory, state which
% sequences have full vs. partial GT, following TUM VI \cite{tumvi}.
