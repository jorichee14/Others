# CSI — ROS 2 CSI publisher for Nexmon (comms/csi)
...
```
comms_msgs/   CsiFrame, CsiStatus
comms/csi/    csi_publisher (arms + publishes), csi_monitor (CLI summary)
```
...
```bash
cd ~/comms_ws
colcon build --packages-select comms_msgs comms
...
```bash
ros2 launch comms csi.launch.py
...
| Topic | Type | Rate |
|---|---|---|
| `~/csi` | `comms_msgs/     CsiFrame` | one per received OFDM frame (~100–500 Hz) |
| `~/status` | `comms_msgs/     CsiStatus` | 0.5 Hz |
| `/mobileN/csi` | `comms_msgs/     CsiFrame` | that transmitter's share of `~/csi` |
