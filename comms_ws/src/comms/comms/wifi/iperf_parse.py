...
Returns a dict with any subset of the following keys populated:
    ...
    interval_mbps (list), bitrate_mbps_min/max/std
"""
...
def _interval_series(d: dict, omit_partial: float = 0.6) -> List[float]:
    """Per-interval goodput (Mbit/s) from an iperf3 JSON test.

    The aggregate in ``end`` hides what happened *inside* the window: a test
    averaging 184 Mbit/s with one 144 Mbit/s second is a different link from
    one steady at 184, and that difference is what predicts a missed deadline.

    Omitted (-O) warm-up intervals are excluded by iperf3 itself. Intervals
    covering less than ``omit_partial`` of the median span are dropped so the
    series is uniform.
    """
    out: List[float] = []
    spans: List[float] = []
    for iv in d.get("intervals", []):
        ...
        sec = s.get("seconds")
        if bps is None or sec is None:
            continue
        out.append(float(bps) / 1e6)
        spans.append(float(sec))
    if not out:
        return []
    med = sorted(spans)[len(spans) // 2]
    return [v for v, sp in zip(out, spans) if sp >= omit_partial * med]


def _series_stats(vals: List[float]) -> Dict[str, object]:
    """min / max / population std of a series. Empty dict if nothing usable."""
    if not vals:
        return {}
    res: Dict[str, object] = {
        "interval_mbps": vals,
        "bitrate_mbps_min": min(vals),
        "bitrate_mbps_max": max(vals),
    }
    if len(vals) >= 2:
        mean = sum(vals) / len(vals)
        var = sum((v - mean) ** 2 for v in vals) / len(vals)
        res["bitrate_mbps_std"] = var ** 0.5
    return res
...
def parse_iperf_json(text: str) -> Dict[str, object]:
    ...
    res.update(_series_stats(_interval_series(d)))
    return res
