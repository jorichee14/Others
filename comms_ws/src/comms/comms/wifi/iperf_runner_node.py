...
bidirectional : bool   publishes to 'wifi/iperf_up' and 'wifi/iperf_down'
                       instead of 'wifi/iperf'. Alternates direction on every
                       test (periodic mode) or
...
bidir_period_s : float continuous+bidirectional: seconds per direction before
                       swapping. Rounded up/down to a whole number of report
                       intervals so every published sample covers the same
                       span. Each swap costs one iperf3 reconnect, which is a
                       real gap in the stream (logged when it happens), so
                       longer segments mean fewer gaps. Default 10.
udp_bitrate_mbps : float  target rate for UDP tests in Mbit/s. 0 = unlimited,
                       which just floods -- the resulting loss only tells you
                       it flooded. -1 = AUTO: run one short TCP probe first
                       and offer udp_rate_fraction of the capacity it
                       measured, so the rate suits this location instead of
                       being guessed from another one.
udp_rate_fraction : float  fraction of probed TCP capacity to offer in auto
                       mode. Default 0.9 -- just under capacity, so loss
                       reflects the link rather than deliberate overload.
udp_probe_s : float    duration of the auto-mode TCP probe. Default 5.
...
from comms_msgs.msg import IperfResult

from comms.wifi import iperf_parse, wifi_parsers
...
class IperfRunnerNode(Node):
    def __init__(self) -> None:
        ...
        # Auto mode (udp_bitrate_mbps < 0): probe TCP capacity here, then
        # offer this fraction of it. Hard-coding a rate measured somewhere
        # else makes UDP loss a property of the guess, not of the link.
        self.declare_parameter("udp_rate_fraction", 0.9)
        self.declare_parameter("udp_probe_s", 5.0)
        ...
        self._udp_auto = self._udp_mbps < 0.0
        if self._udp_auto:
            self._udp_mbps = 0.0          # until the probe resolves it
        self._udp_frac = gp("udp_rate_fraction").get_parameter_value().double_value
        if not 0.0 < self._udp_frac <= 1.0:
            self._udp_frac = 0.9
        self._udp_probe_s = gp("udp_probe_s").get_parameter_value().double_value
        ...
        # Bidirectional runs publish each direction on its own topic, so a
        # subscriber or a bag can take one direction without filtering on the
        # 'reverse' field, and `ros2 topic hz` reports a real per-direction
        # rate. Single-direction runs keep the plain topic.
        if self._bidir:
            self._pub = None
            self._pub_up = self.create_publisher(
                IperfResult, "wifi/iperf_up", qos
            )
            self._pub_down = self.create_publisher(
                IperfResult, "wifi/iperf_down", qos
            )
            self._topics = "'wifi/iperf_up' + 'wifi/iperf_down'"
        else:
            self._pub = self.create_publisher(IperfResult, "wifi/iperf", qos)
            self._pub_up = self._pub_down = None
            self._topics = f"'{self._pub.topic_name}'"
        ...
        self._last_sample_t = 0.0  # for measuring the bidir swap gap
        ...
        self.get_logger().info(
            ...
            f"{direction}, iface "
            f"'{self._iface or '<none>'}' -> topic {self._topics}."
        )

    # ----------------------------------------------------------------------
    def _publish(self, msg: IperfResult) -> None:
        """Send on the topic matching the message's own direction.

        msg.reverse is authoritative rather than self._cur_reverse: a message
        built before a flip must land on the topic for the direction it was
        actually measured in.
        """
        if self._pub is not None:
            self._pub.publish(msg)
        elif msg.reverse:
            self._pub_down.publish(msg)
        else:
            self._pub_up.publish(msg)
    ...
    def _loop(self) -> None:
        ...
        while not self._stop.is_set() and rclpy.ok():
            ...
            if self._continuous:
                ...
                # Swap immediately between healthy bidir segments -- the
                # reconnect is already the gap in the sample stream, so any
                # extra sleep widens it. Back off only on failure, so a dead
                # server is not hammered.
                if not (self._bidir and healthy):
    ...
    def _new_msg(self, success: bool = False, error: str = "") -> IperfResult:
        ...
        msg.bitrate_mbps_min = NAN
        msg.bitrate_mbps_max = NAN
        msg.bitrate_mbps_std = NAN
        return msg

    def _build_cmd(self) -> list:
        # -i is explicit so the per-interval series in the JSON has a known
        # granularity; without it the reporting interval is iperf3's default
        # and interval_mbps would mean something different per version.
        cmd = [
            "iperf3", "-c", self._server, "-p", str(self._port),
            "-t", str(self._duration), "-i", f"{self._cont_interval:g}", "-J",
        ]
    ...
    def _probe_tcp_capacity(self) -> None:
        """Measure TCP capacity once, then set the UDP offered rate from it.

        Runs before the first UDP test in auto mode. Not published: this is a
        calibration step, not a measurement, and mixing a TCP probe into a UDP
        run would put two protocols on one topic.
        """
        cmd = [
            "iperf3", "-c", self._server, "-p", str(self._port),
            "-t", f"{self._udp_probe_s:g}", "-O", "1", "-J",
        ]
        if self._connect_timeout > 0:
            cmd += ["--connect-timeout", str(self._connect_timeout)]
        if self._parallel > 1:
        ...
        try:
            out = subprocess.run(
                ...
                timeout=self._udp_probe_s + 15.0, check=False,
            )
        except (OSError, subprocess.TimeoutExpired) as exc:
            self.get_logger().warn(
                f"UDP auto-rate probe failed ({exc}); falling back to "
                "unlimited, so loss will reflect flooding.")
            self._udp_auto = False
            return
        parsed = iperf_parse.parse_iperf_json(out.stdout or "")
        cap = parsed.get("bitrate_mbps") if parsed.get("success") else None
        if not cap or cap <= 0 or math.isnan(float(cap)):
            self.get_logger().warn(
                "UDP auto-rate probe returned no capacity; falling back to "
                "unlimited.")
            self._udp_auto = False
            return
        self._udp_mbps = float(cap) * self._udp_frac
        self._udp_auto = False
        self.get_logger().info(
            f"UDP auto-rate: TCP probe measured {float(cap):.1f} Mbit/s here, "
            f"offering {self._udp_mbps:.1f} Mbit/s "
            f"({self._udp_frac:.0%} of capacity).")

    def _run_once(self) -> IperfResult:
        if self._udp_auto and self._proto == "udp":
            self._probe_tcp_capacity()
        msg = self._new_msg()
        ...
        self.get_logger().info(
            ...
            + (f" [{msg.bitrate_mbps_min:.0f}-{msg.bitrate_mbps_max:.0f}]"
               if not math.isnan(msg.bitrate_mbps_min) else "")
    ...
    def _segment_s(self) -> float:
        """Bidir segment length, rounded to a whole number of report intervals.

        An unaligned segment (e.g. 10 s at 0.3 s reports) ends mid-interval and
        iperf3 emits a short stub report, which would land in the stream as a
        sample covering less time than all the others. Rounding keeps every
        published sample the same width.
        """
        n = max(1, round(self._bidir_period / self._cont_interval))
        return n * self._cont_interval

    def _build_continuous_cmd(self) -> list:
        # -t 0 runs until we kill it; -i 1 emits a report every second.
        # Bidirectional: run one aligned segment per direction instead, so the
        # loop restarts us with the direction flipped.
        seg = f"{self._segment_s():g}" if self._bidir else "0"
        cmd = [
            "iperf3", "-c", self._server, "-p", str(self._port),
    ...
    def _run_continuous(self) -> bool:
        ...
        arrow = "down" if self._cur_reverse else "up"
        seg = f" ({self._segment_s():g}s segment)" if self._bidir else ""
        self.get_logger().info(
            f"continuous iperf {self._proto.upper()} {arrow} started{seg}."
        )
        published = False
        last_line = ""  # iperf3's error text, for when no samples arrive
        # Shortest report we accept. iperf3 emits a stub interval at connect
        # and (on an unaligned -t) at the end; publishing those would put
        # samples of unequal width into an otherwise fixed-rate stream.
        min_span = 0.6 * self._cont_interval
        try:
            while not self._stop.is_set() and proc.poll() is None:
                ...
                if parsed is None:
                    continue
                span = float(parsed.get("seconds", self._cont_interval))
                if span < min_span:
                    continue          # partial report: not a full sample
                ...
                self._publish(msg)
                if not published and self._bidir and self._last_sample_t:
                    gap = time.monotonic() - self._last_sample_t
                    self.get_logger().info(
                        f"direction swap cost {gap:.2f}s of samples."
                    )
                published = True
        ...
        if not published and not self._stop.is_set():
            ...
            self._publish(self._new_msg(success=False, error=err[:200]))
        return published
    ...
    def _apply(msg: IperfResult, parsed: dict) -> None:
        ...
        if "interval_mbps" in parsed:
            msg.interval_mbps = [float(v) for v in parsed["interval_mbps"]]
        for f in (
            "bitrate_mbps", "rtt_ms_mean", "rtt_ms_min", "rtt_ms_max",
            "jitter_ms", "lost_percent",
            "bitrate_mbps_min", "bitrate_mbps_max", "bitrate_mbps_std",
        ):
            if f in parsed:
