...
- **Still needed: one measurement.** The robots' half-extents, for the
  agents-as-objects pseudo-labels. `load_config` now fails on exactly that and
  nothing else; with placeholder extents the config loads cleanly as three
  agents (ego `mobile_1`, RSU `infra_1`) with clock reconciliation on.
