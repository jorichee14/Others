...
## Phase 8 — Real testbed data (`../rosbag_to_opv2v/`)

> **Moved (2026-09-04).** The converter, its scripts, its tests, the MIRC config
> and `docs/ROS2OPV2V.md` now live in their own top-level project,
> [`../rosbag_to_opv2v/`](../rosbag_to_opv2v/), with their own README,
> requirements and gitignore. It depends on nothing in this folder. Paths below
> that read `ros2opv2v/…`, `scripts/convert_rosbag.py`, `configs/mirc_coop2.yaml`
> or `docs/ROS2OPV2V.md` are relative to that project from here on; the history
> is kept as written.
...
- **Ground truth (Step 8.3):** the scene's two chairs are static, so they are
  labelled once in the map frame with `scripts/label_static.py` and written into
  every frame — see the 8.3 log entry. `object.emit` stays off; agent-derived
  boxes are right by construction and test nothing, while a hand-placed chair
  checks the whole geometry chain.
...
| Date | Step | Notes |
|------|------|-------|
| 2026-09-05 | 8.3 ✅ | **Ground truth exists: two chairs, labelled, checked, converted.** `chair_1` 0.80 x 0.73 x 0.96 m at [3.104, -1.432, -0.237] yaw 158.2 deg and `chair_2` 0.67 x 0.60 x 0.87 m at [4.380, -8.094, -0.283] yaw 2.0 deg, in all 1330 frames of all three agents; `validate_opv2v.py` **PASSED, 0 warnings**, 2.00 GT boxes per frame inside the ego's range. `--check` is the point of the exercise and it passed: boxes fitted against the map WITHOUT reference to any agent pose are hit by `mobile_1`'s returns in 89-100% of sampled frames, which tests the extrinsics, the anchoring and the frame timing at once. `mobile_2`'s 33% is field of view, not error — the tell is that when it sees a chair it puts a median 1739 points inside it against `mobile_1`'s 39: a misplaced box gives a trickle in many frames, a narrow-FOV depth camera a flood in a few. `infra_1` at 0% is a ~49-point radar 12-18 m away. Floor is z = **-0.719** in the coop2 map, so `ground_lift` is 0.93 / 1.03 for the carts if OPV2V-pretrained weights are ever run; 0 otherwise. Getting here cost four wrong answers, each now a test: a scanned CEILING is the tallest return over every cell beneath it, so it hides all the furniture in a height map (and bridges a chair to the roof through anything vertical beside it); a FLOOR is a slab centimetres deep, so a 3 cm cluster gate lets a chair's legs connect the fill to the whole ground plane and the box becomes the search radius, 2.4 m across and 10 cm tall; a TRUNCATED PCD read as `count=min(n, payload/itemsize)` returned a tenth of its own header's point count in silence, which is indistinguishable from a map whose furniture was filtered out and sent the search after the wrong problem; and a re-fit APPENDED rather than replaced, so re-running two seeds left four labels — a duplicate is a phantom object in every frame that scores as a miss against exactly the detectors that got the scene right. Wall-vs-chair is now `wall_contact`, the fraction of a blob's outline that is structure (a chair reads 0%, one against a wall ~30%, a wall's own foot 50%+), and `--map-image` draws the searched floor from above — heights, structure, driven path, numbered boxes — written from zlib and struct because installing a plotting stack on the robot to look at ten blobs is not a fair trade. Tests 90/90 -> **106/106**. |
| 2026-09-04 | 8.3 | **Ground truth from the run's own map: static objects labelled once.** The scene's two chairs never move, so each has ONE pose for the whole recording — labelled once in the map frame and written into all 1330 frames of all three agents, already projected, since every frame's pose is known. Two boxes instead of 2x1330. It is also better GT than an agent-derived box, which is right by construction and therefore tests nothing: a hand-placed chair is an INDEPENDENT fact, so `--check` (do both agents' real returns fall inside it?) tests the extrinsics, the anchoring and the frame timing at once. `ros2opv2v/statics.py` (general PCD reader incl. binary_compressed/LZF, floor detection, voxel connected-components clustering, min-area-rectangle box fit via rotating calipers) + `scripts/label_static.py` (verify / propose / fit / check) + `output.labels_file` wired through `merge_external_labels`. The box is FITTED, not dragged: the seed need only land on the object, yaw comes from the hull exactly, roll/pitch are held at zero, and the box is extended to the detected floor because a lidar sees a chair's seat and not its legs. Two checks nothing else can do: the map cloud's frame (a pre-anchor cloud looks equally plausible and puts every box metres out — tested by whether the MOVING agents' trajectories lie inside its footprint) and the floor's z, which is exactly the `ground_lift` input no pose can supply. **Caught the same mistake I made on `infra_1`'s world_pose**: the first frame check failed on the static RSU, which legitimately sits outside the mapped floor looking into it; only moving agents are evidence. Tests 80/80 -> **90/90**. |
...
| 2026-09-04 | 8 | **Converter separated from the perception analysis.** `ros2opv2v/`, the three stage scripts, the 67 self-tests, `configs/mirc_coop2.yaml` and `docs/ROS2OPV2V.md` moved (with history, via `git mv`) to a new top-level project `rosbag_to_opv2v/` with its own README, `requirements.txt` (numpy, pyyaml, mcap, mcap-ros2-support) and `.gitignore`. It imports nothing from this folder; the few prose references to `results/ANALYSIS.md`, `configs/matrix.yaml` and `commchannel/` were re-pointed across the boundary. Tests 67/67 from the new location without code changes — the scripts already located the project root relative to themselves. |
