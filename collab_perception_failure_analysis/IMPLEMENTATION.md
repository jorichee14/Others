...
- **Still needed: one measurement.** The robots' half-extents (and where the
  camera sits relative to the robot's centre), for the agents-as-objects
  pseudo-labels. `load_config` fails on exactly that and nothing else; with
  placeholders the config loads cleanly as three agents (ego `mobile_1`, RSU
  `infra_1`) with clock reconciliation on.
